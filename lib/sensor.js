...
class Sensor extends EventEmitter{
    ...
    constructor(options){
        ...
        this.host = os.hostname();//this.sanitize(cp.execSync('cat /etc/hostname'));
    ...
    /**
     * Watches file constantly using: tail -f
     */
    runTailWatch(file,cb){
        let p = cp.spawn('tail', ['-0f',file]);
        p.stdout.on('data',(data)=> {
            data = data.toString().split("\n").filter(a=>a);
            let output = [];
            for(let i in data) output.push(cb(data[i]));
            this.out(output);
        });
    }

    /**
     * Watch all logins and attempts
     */
    runLogWatcher(file,name,rowProcessor){
        this.runTailWatch(file,(row) => {
            if(!rowProcessor) rowProcessor = this.rowProcessor;
            let processed = rowProcessor(row);
            return [this.spath + '.' + name + '.' + processed.metric , processed.value + '', processed.time];
        });
    }

    /**
     *  Simple row processor used for file watcher;
     */
    rowProcessor(row){
                let now = parseInt(Date.now() / 1000) + '';
                let metric = row.substr(row.indexOf(': ') + 2,row.length).toLowerCase().replace(/[^a-z ]/gi,' ');
                return {
                    metric:metric,//.match(/\b(\w)/g).join(''),
                    value:1,
                    time:now
                };
    }
    ...
    runDelta(){
        ...
        let cores = os.cpus().length; //this.num(cp.execSync('nproc')); // get number of cores
